...
{
	*(volatile unsigned int*)(0x8000020000000000ULL|addr) = value;
}

static inline unsigned int read32(unsigned int addr)
{
	return *(volatile unsigned int*)(0x8000020000000000ULL|addr);
}

static inline void write32mem(unsigned int addr, unsigned int value)
{
	*(volatile unsigned int*)(0x8000000000000000ULL|addr) = value;
}

static inline unsigned int read32mem(unsigned int addr)
{
	return *(volatile unsigned int*)(0x8000000000000000ULL|addr);
}
...
static void tx_init(struct enet_context *context, void *base)
{
	...
	dcache_flush(context->tx_descriptor_base, TX_DESCRIPTOR_NUM * 0x10);
...
err_t enet_init(struct netif *netif)
{
	...
	if(context == NULL) {
		...
		//TODO Read flash page for mac address
